...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        return grad_output * sigmoid_out * (-sigmoid_out + 1.0)
...
class Log(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        (t1,) = ctx.saved_values
        # d(log(x))/dx = 1/x, so grad = grad_output / t1
...
class Exp(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        (t1,) = ctx.saved_values
        # d(exp(x))/dx = exp(x), so grad = grad_output * exp(t1)
...
class Permute(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        # Compute inverse permutation: if order[i] = j, then inverse[j] = i
        inverse_order = [0] * len(order_tuple)
        for i, pos in enumerate(order_tuple):
            inverse_order[pos] = i
